...
async def alembic_migrations(postgres_service: PostgresContainer) -> None:
    ...
    # env.py uses an async engine (asyncpg is this repo's only DB driver), so the
    # +asyncpg dialect is required here too.
    db_url = f"postgresql+asyncpg://{username}:{password}@{host}:{port}/{database}"

    # SQLALCHEMY_DATABASE_URL is the name alembic/env.py expects.
    env = os.environ.copy()
