...
- [ ] Verhaltens-Specs unter `contexts/diary/specs/get_day/`: GetDay liefert bei verfuegbarem Gateway echte activities-Eintraege (name/detail/kcal); ohne Gateway ein leeres Array; **Diese Specs laufen mit In-Memory-Fake des Gateways, ohne Datenbank, ohne Container**
...
- [ ] **Contract-Test** (siehe docs/milestones/02-test-pyramide.md, Form A): Diary definiert eine implementierungsunabhaengige Test-Suite `assert_health_activity_gateway_contract(gateway)` unter `contexts/diary/specs/contracts/`, HealthSync importiert sie und fuehrt sie gegen seinen eigenen In-Process-Adapter aus
