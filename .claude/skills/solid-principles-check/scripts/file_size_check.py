...
The mechanical pre-check for solid-principles-check: an oversized file/type is
a classic Single-Responsibility smell, so run this first and treat OVER/CROSSED
rows as SRP candidates rather than eyeballing size by hand.
...
thermo-nuclear-code-quality-review's changed_files.py wraps the same module,
so the one objective rule has a single source of truth across both skills.
"""
