...
The mechanical pre-check for verifier-large-class: "a class contains many
fields/methods/lines of code" (refactoring.guru) is, at the file level this repo
uses (one class per file, per the Vertical Slice convention), a length signal —
so run this first and treat OVER/CROSSED rows as candidates worth opening rather
than eyeballing size by hand. Tuned between verifier-long-method's 60 (one method
dominating a file) and solid-principles-check's 1000 (SRP's much coarser net): a
flag here plausibly means "this one class has outgrown its single concept", not
"this file merely has several methods". Still just a proxy — a flagged file may
turn out to be one cohesive concept with a long enumeration/mapping table, which
is not a Large Class finding; the skill's own judgment decides which.
...
(or 300 if that is missing/unreadable); --threshold overrides it.

The measurement/classification logic lives in _shared/file_size_check.py — this
is the third skill (after solid-principles-check and verifier-long-method) to
wrap that module, so the one objective rule has a single source of truth.
"""
...
def main() -> int:
    ...
    threshold = args.threshold if args.threshold is not None else int(load_config(CONFIG).get("file_size_warn_lines", 300))
