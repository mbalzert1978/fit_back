...
2. **Task.md erzeugen** — Ticket-Body in einen gehärteten Task-Prompt umformen
   ...
   Pipeline-Arbeitsartefakt: **muss gitignored bleiben**, nie versehentlich commiten.
...
Grund, warum der Nutzer das explizit so wollte.

**Bekannter Workflow-Tool-Bug:** `args` kommt manchmal als `undefined` im Skript an
(reproduziert, ungeklärt). Workaround: Ticket-/Eingabedaten als literales `const`
direkt im Skript-Body hardcoden, nicht auf `args` verlassen — siehe
`docs/reflections/exp_workflow-tool-args-bug.md`.

**Kritische Prompt-Regel:** jeder `agent()`-Aufruf, der Git-Kommandos in einem
Worktree ausführt, braucht eine **explizite Anweisung, zuerst dorthin zu wechseln**
(`cd <pfad>` als erster Schritt), nicht nur den Pfad als Kontext-Satz — sonst
committet ein Agent versehentlich im Haupt-Checkout direkt auf `main`, bricht
QA/Security/PR-Review vollständig aus und du musst per Revert bereinigen. Siehe
`docs/reflections/exp_workflow-agent-cd-explizit.md` und
...
Vor jedem PowerShell- oder CI-lastigen Schritt kurz gegenprüfen:
...
## Entscheidungen & Memory-Policy

**Kein externes/persistentes Memory** — weder Claude Codes sitzungsübergreifendes
Memory-System noch sonst eine Notiz-Ablage außerhalb des Repos. Entscheidungen
gehören ausschließlich unter `docs/decisions/` (datiert, mit Uhrzeit), destillierte
wiederverwendbare Lektionen unter `docs/reflections/` (siehe oben) — beides normale,
versionierte Repo-Dateien, committet wie jede andere Änderung.

## Sprache

Alle Dokumentation und Projektnotizen (Issues, Decisions, Reflections, Milestones,
PR-Beschreibungen für dieses Repo) auf **Deutsch** — ausgenommen generische,
repo-übergreifende Infrastruktur (`.claude/skills/`-Definitionen und deren
`config.json`) sowie Code/Bezeichner/Kommentare selbst.
